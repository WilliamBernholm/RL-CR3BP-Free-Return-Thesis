...
class TimeAwareRecurrentRolloutBuffer(RolloutBuffer):
    """
    ...
    def reset(self):
        ...
        # Time-aware PPO: per-transition elapsed-time ratio relative to cfg.dt
        self.dt_ratios = np.ones((self.buffer_size, self.n_envs), dtype=np.float32)

    def add(self, *args, lstm_states: RNNStates, dt_ratio: np.ndarray, **kwargs) -> None:
        """
        ...
        self.dt_ratios[self.pos] = np.array(dt_ratio, dtype=np.float32)

        super().add(*args, **kwargs)

    def compute_returns_and_advantage(self, last_values: th.Tensor, dones: np.ndarray) -> None:
        """
        Semi-Markov / duration-aware GAE with:
            gamma_t = gamma ** dt_ratio_t
            lambda kept step-based

        This matches the paper convention more closely:
            A_t = delta_t + lambda * gamma_t * A_{t+1}

        Current TD residual convention used here:
            delta_t = r_t + gamma_t * V_{t+1} - V_t

        So:
        - gamma is time-aware
        - lambda is step-aware
        """
        ...
        for step in reversed(range(self.buffer_size)):
            ...
            dt_ratio = np.maximum(self.dt_ratios[step].astype(np.float32), 0.0)

            # time-aware gamma only
            gamma_eff = np.power(self.gamma, dt_ratio)

            # TD residual
            delta = self.rewards[step] + gamma_eff * next_values * next_non_terminal - self.values[step]

            # step-aware lambda, time-aware gamma
            last_gae_lam = delta + self.gae_lambda * gamma_eff * next_non_terminal * last_gae_lam
